...
(def lib 'io.github.hive-agi/hive-cppb)
...
(defn coord-patterns
  ...
  (let [l (java.util.regex.Pattern/quote (str lib))]
    [;; deps.edn:   io.github.hive-agi/hive-cppb {:mvn/version "X"}
     (re-pattern (str "(" l " \\{:mvn/version \")([^\"]+)(\")"))
     ;; Leiningen:  [io.github.hive-agi/hive-cppb "X"]
     (re-pattern (str "(\\[" l " \")([^\"]+)(\")"))]))
...
(def pom-data
  [[:description "CPPB pattern macros (Collect → Promote → Pipeline → Boundary): a tiny Clojure macro layer codifying a stratified architecture for data-enrichment workflows, with railway-oriented Result threading via hive-dsl."]
   [:url "https://github.com/hive-agi/hive-cppb"]
   [:licenses
    [:license
     [:name "MIT"]
   ...
   [:scm
    [:url "https://github.com/hive-agi/hive-cppb"]
    [:connection "scm:git:git://github.com/hive-agi/hive-cppb.git"]
    [:developerConnection "scm:git:ssh://git@github.com/hive-agi/hive-cppb.git"]
    [:tag (str "v" version)]]
   [:developers
    [:developer
